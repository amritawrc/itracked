...
class Org_managestu extends MY_OrgController {
    ...
    public function index() {
        ...
        $details = $this->org->fetch('student', $cond3);
        //t($details,1);
...
//        echo $this->db->last_query();
    ...
    function manage_ven_ev() {
        ...
        $arra['map'] = 1;
        //t($arr,1);
        $this->org->add('ins_stu_map', $arr);
        $cond1 = " AND `stu_seq_no`='{$stuid}'";

        $this->org->edit_cond('student', $arra, $cond1);
...
//        $details = $this->org->fetch('address', $cond);
        ...
        $cond = "AND status!='5' and stu_seq_no='$stu_seq_no'";
        $details1 = $this->org->fetch('student', $cond);
        ...
        $cond = "AND status!='5' and org_seq_no='$org'";
        ...
        $chk1 = $this->org->edit_cond('student', $arr, $cond);
...
//        $arra['org_id'] = $org_id; 
            ...
            $this->org->edit_cond('ins_stu_map', $gr, $cond1);

            $this->org->edit_cond('grp_stu_map', $st, $cond1);

            $cond1 = " AND `status`!=5 AND `stu_seq_no`='{$id}'";

            $chk1 = $this->org->edit_cond('student', $arra, $cond1);
            ...
            $details = $this->org->fetch('city');
            $this->data['dets'] = $details;
            $cond3 = "AND status!='5' and stu_seq_no='$id'";
            $details = $this->org->fetch('student', $cond3);
            //t($details,1);
            foreach ($details as $key => $value) {
                 $email = $value['email'];
...
//                $details[$key]['phone'] = $org_details[0]['phone'];
    ...
    function delete($id, $orig) {
        ...
        if (!empty($id)) {
            ...
            $arr['status'] = 5;
            $cond = " and stu_seq_no='{$id}'";
            $this->org->edit_cond('student', $arr, $cond);
            $cond1 = " and id='{$org}'";
            $this->org->edit_cond('address', $arr, $cond1);
            
            $cond1 = " and stu_id='{$id}'";
            $this->org->delete_cond('ins_stu_map',$cond1);
    ...
    function event_det($stu_id = "") {
        ...
        $dett = $this->org->fetch('ins_stu_map', $cond4);
        //echo $this->db->last_query(); 
    ...
    function studentPanic($param) {
        ...
        $sql = "select stu_id from tbl_ins_stu_map where org_id=$org_id AND (panic=1 and seen=0) OR (panic=0 and seen=1)" ;
...
//        exit;
        foreach ($panic_student as $key => $value) {
            ...
            $cond = "AND stu_seq_no='" . $value['stu_id'] . "' and status!='5'";
            $panic_student_details = $this->org->fetch('student', $cond);
            
            $cond1 = "AND stu_id='" . $value['stu_id'] . "' and status!='5'";
            $dett = $this->org->fetch('ins_stu_map', $cond1);
            ...
            $panic_student_details[0]['event_name'] = $this->event_det($panic_student_details[0]['stu_seq_no']);
            $panic_student_details[0]['event_id'] = $this->event_det1($panic_student_details[0]['stu_seq_no']);
    ...
    function panicStudentLocation($stu_seq_no) {
        ...
        $sql = "select map.*,stu.*,tbl_event.event_name,tbl_instructor.first_name as instructor_first_name,tbl_instructor.last_name as instructor_last_name from tbl_ins_stu_map as map inner join tbl_student as stu on map.stu_id=stu.stu_seq_no inner join tbl_event on tbl_event.ev_seq_no=map.ev_id inner join tbl_instructor on tbl_instructor.ins_seq_no=map.ins_id where map.org_id=$org_id AND map.panic=1 AND map.lat!=''";
        $query = $this->db->query($sql);
...
//         t($row1);
        $select = "ins_seq_no,first_name,last_name,email,exp as age_on_return_to_uk,phno as mobile,lat,long";
        $cond = "AND lat!='' and status!='5'";
        $ins_details = $this->org->fetch('instructor',$cond,$select);
//        t($ins_details);
//        exit;
        foreach ($ins_details as $key => $value) {
            $ins_details[$key]['type']='I';
        }
...
//        $sql = "update tbl_ins_stu_map set panic=0,seen=1 where org_id=$org_id";
//        $query = $this->db->query($sql);
...
//        exit;
    ...
    function panicStudentMessages($id) {
        ...
        $details = $this->org->fetch('ins_stu_map', $cond);
    ...
    function panicStudentImages($id) {
     ...
     function panicStudentseen($id) {
        ...
        $arr['panic'] = '0';
        $arr['seen'] = '1';
        $this->org->edit_cond('ins_stu_map', $arr, $cond);
