...
// 	t($details , 1);
    ...
    function auto_payment($org_id = ''){
        ...
        $query_event = $this->db->query($sql_event);
        $all_event_in_an_org = $query_event->result_array();
        $org_email = $all_event_in_an_org[0]['username'];
        $org_name = $all_event_in_an_org[0]['org_name'];
            // t($all_event_in_an_org); exit;
    foreach($all_event_in_an_org as $key => $value){
...
// exit;
    ...
    if($org_currency == 'USD'){
       $data_invoice = array(
        ...
        $this->invoiceMail($invoice_number, $org_name, $org_email, $org_id);
 ...
 return true;
    ...
    function view_invoice($org_id = ''){
        ...
        LEFT JOIN tbl_org as org ON org.org_seq_no = inv.org_id WHERE inv.org_id = '" . $org_id . "' AND eve.event_billing = '1'";
        $query = $this->db->query($sql);
        $all_events_invoiced = $query->result_array();

        $event_seq_no = $all_events_invoiced[0]['ev_seq_no'];
        $no_of_students = $all_events_invoiced[0]['no_of_students'];
        // $invoice_generation_date = $all_events_invoiced[0]['created_date'];
    // t($all_events_invoiced); exit;
    ...
    foreach($all_events_invoiced as $key => $value){
         ...
         $sql_student = "SELECT usr.fname, usr.lname, usr.type, tevs.stu_id, eve.ev_seq_no, DATEDIFF('$end_date', '$start_date') as duration "
                ...
                ."LEFT JOIN tbl_event_stu_map as tevs ON usr.id = tevs.stu_id "
                ."LEFT JOIN tbl_event as eve ON eve.ev_seq_no = tevs.ev_id "
                ."WHERE eve.ev_seq_no = '" . $event_seq_no . "' AND eve.event_billing = '1' AND eve.org_id = '" . $org_id . "' ";
        $query_student = $this->db->query($sql_student);
        $all_student_in_an_event = $query_student->result_array();
    }
...
//   exit;  
        ...
        $this->data['all_student_in_an_event'] = $all_student_in_an_event;
        $this->data['no_of_students'] = $no_of_students;
        $this->get_include();
        $this->load->view($this->viewDir . "invoice/invoice", $this->data);
    }
    function invoiceMail($invoice_number, $org_name, $org_email, $org_id){
        $org_id = base64_encode($org_id);
        
      $link_url = $this->data['base_url'].'invoice/view_invoice'; 
      $to = $org_email;
      $admin_email = 'devosmita@wrctechnologies.com';

      $message_tpl = $this->load->view($this->viewDir .'email/invoiceEmail','',true);
      
        $patterns = array();
        $patterns[0] = '/!!!#org_name#!!!/';
        $patterns[1] = '/!!!#link_url#!!!/';
        $patterns[2] = '/!!!#invoice_number#!!!/';
        
        $replacements = array();
        $replacements[0] = ucfirst($org_name);
        $replacements[1] = $link_url . '/' . $org_id;
        $replacements[2] = $invoice_number;
        
        $message_content = preg_replace($patterns, $replacements, $message_tpl);
        
        $subject = 'Invoice:' . $org_name;
        
         $chkEmailStatus = $this->email_send($to, $subject, $message_content ,0,1);
         $chkEmailStatusAdmin = $this->email_send($admin_email, $subject, $message_content ,0,1);
 		 //echo 'ddggdg-'.$chkEmailStatus;exit;
        
        if($chkEmailStatus && $chkEmailStatusAdmin){
        	return 1;
        }else{
        	return 0;
        }
